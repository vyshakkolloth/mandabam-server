...
const acceptEnquire= async(req,res)=>{
    try {
      ...
      const result= await BookingModel.findByIdAndUpdate(id, { $set: { "status": true,amount:amount } });
...
const paymentVerify=async(req,res)=>{
    try {
		...
		if (razorpay_signature === expectedSign) {
			await BookingModel.findByIdAndUpdate(id, { $set: {payment:true,paymentId:razorpay_payment_id } });
 ...
 const bookedVenue= async(req,res)=>{
	try{
		const id=  req?.userId
		const data=await  BookingModel.find({userId:id,payment: true }).populate({
			path: "venueId",
